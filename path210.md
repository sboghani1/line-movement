...
fademorocco
5 days out, france -1 +101 is great value for best team of the tournament. they should win by at least 2 so worst case this is a push. 4 days out, france is -1 +102 and the total is o2.5 +101. 3 days out, france is -1 +102 and the total is o2.5 +103. 2 days out, france is -1 +108 and the total is o2.5 +101. 1 day out, france is -1 +102 and the total is o2.5 +100. 3 hours out, france is -1 +104 and the total is o2.5 +108. 1 hour out, france is -1 +112 and the total is o2.5 -102. trent is on u2.5. tempting to parlay france -.5 with mbappe shots on goal -334 (it went up from -334 yesterday) to get a -114 payout rather than taking a push if france only wins by 1. 90 minutes minutes out, france is -1 +110 and the total is o2.5 +100. 1 hour out, france is -1 +107 and the total is o2.5 -102. seems like all the tipsters are on france to either advance or -.5. 30 minutes out, france is -1 +100 and the total is o2.5 -112. 20 minutes out, france is -1 +100 and the total is o2.5 -110.  
final_lean:
tags: back_favorite, follow_line_movement, faded_tipster, follow_consensus, situational_angle, overconfidence, decision_day_before, parlay_conflict, chased_better_payout, total_over
<ins>direction: TOTAL (over o2.5): SMALL-MODERATE LEAN FOR (primary play) — the over has firmed cleanly and steadily +108 (3h) -> -102 -> -112 -> -110 (money onto the over = follow_line_movement, 3/3), and trent (single tipster) on the UNDER is a fade signal (faded_tipster 2/1) that agrees. SIDE (france -1): now SMALL LEAN FOR (upgraded) — the price finally tightened ONTO france late +112 -> +110 -> +107 -> +100 -> +100 (money onto the favorite), which now WITH the broad tipster consensus is mild follow_line_movement/follow_consensus support. CAUTION: the two plays mildly CONFLICT (a 2-0 france win covers -1 but stays UNDER 2.5), so don't treat them as independent. PARLAY (france -.5 + mbappe -334): AVOID — unchanged.</ins>
strength: small-moderate on the over (primary), small on france -1 (now supported but conflicts with the over), hard avoid on the parlay.
reasoning: the late data flips the SIDE read from my prior take and firms the total. (1) SIDE: france -1 went +101 (5d) -> ... -> +112 (1h) -> +110 -> +107 -> +100 -> +100 (20m): the last ~90 minutes are a steady one-direction tightening ONTO france (the plus price shrank +112 -> +100), not a spike-and-revert, so this is genuine late money onto the favorite (follow_line_movement) — and it now moves WITH the "all tipsters on france" consensus rather than against a flat line, so the fade_consensus tell from my previous read no longer holds. That upgrades france -1 to a SMALL lean FOR. Two cautions keep it small: the market still only has france at a pick'em -1 (+100), so it does NOT see the "win by 2" blowout the day-5 thesis imagined (situational_angle 7/13, overconfidence 3/7, decision_day_before 2/5) — it sees france winning a CLOSE one; and the -1 pushes on a 1-goal win. (2) SIDE/TOTAL CONFLICT: per the notes a side and total on the same game are unlikely to both be right, and here they actively fight — the premise "france wins by 2" points at a 2-0/2-1 type result, but a 2-0 win is UNDER 2.5, so backing france -1 AND the over is mildly self-conflicting; size for that, don't double up as if independent. (3) TOTAL is the cleaner, longer-standing signal: the over firmed from +108 three hours out down to -110 and held, a sustained follow_line_movement move, reinforced by fading trent's under — so it is the PRIMARY play (still small-moderate; total_over is 3/6). (4) PARLAY: still avoid — mbappe shots -334 bolted onto france -.5 to reach -114 is manufactured payout with a new failure point (parlay_conflict 0/2 + chased_better_payout 0/10); take france -.5 straight if you want to erase the push. net: over is the primary small-moderate play (clean firm + fade trent), france -1 is now a SMALL lean too (late money + consensus came onto it) but recognize it conflicts with the over on a 2-0 line, and hard-avoid the parlay.